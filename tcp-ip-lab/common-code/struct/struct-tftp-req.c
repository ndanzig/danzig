/* FILE: struct-tftp-req.c -- This program checks the following
   ...
   The example structure is similar to a TFTP read-request or
...
int main ()
{
  ...
  union {
    ...
    /* Note that this struct is similar to the structure of TFTP read-
       & write-request packets: 1 16-bit unsigned number, followed by a
       byte array of data.  I chose the names of the fields based on
       this similarity.
    */
